...
# Compatible with the default python install on OS X
# 
# Install the dependencies matplotlib for python
...
usage = """
    Usage: ./graph.py {path_to_csv_files}
    
    Example ./graph.py /tmp/Magento
"""
...
for infile in glob.glob( os.path.join(path, '*summary-report.csv') ):
    file_parts = infile.split('-')[0]
    thread_parts = file_parts.split('/')
    threads = int(thread_parts.pop())
    for row in csv.DictReader(open(infile)):
        if (int(row['elapsed']) < 1):
            row['elapsed'] = 1
...
for label in elapsed:
  ...
  plot_labels = []
  column = 1
  keys = elapsed[label].keys()
  for thread_count in sort(keys):
